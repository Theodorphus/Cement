...
export default function ProductGallery({ images }: { images: ProductImage[] }) {
  ...
  return <div className="product-gallery">
    <div className="product-main-image"><Image key={current.src} src={current.src} alt={current.alt} fill sizes="(max-width: 860px) 100vw, 560px" priority style={{objectFit:"scale-down"}} />{images.length>1&&<span className="gallery-position" aria-hidden="true">{String(selected+1).padStart(2,"0")} <span>/ {String(images.length).padStart(2,"0")}</span></span>}</div>
