export const CONTACT_LIMITS = { namn: 100, epost: 254, telefon: 40, meddelande: 5000, website: 200 };
export type ContactFields = { namn: string; epost: string; telefon: string; meddelande: string; website: string };
export function validateContact(input: unknown): { fields?: ContactFields; error?: string } {
  ...
  const fields: ContactFields = { namn: "", epost: "", telefon: "", meddelande: "", website: "" };
  ...
  // Telefon är frivilligt, men om det fylls i ska det gå att ringa.
  if (fields.telefon && !/^[+0-9][0-9\s\-()]{5,}$/.test(fields.telefon)) return { error: "Ange ett giltigt telefonnummer, eller lämna fältet tomt." };
  return { fields };
}
