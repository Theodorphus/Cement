...
export default async function CategoryPage({params}:Props){
 ...
 return <div className="page-mount">
  ...
  <div className="content-page">
   {products.length>0&&<div className="subcategories">{products.map(p=><Link href={productPath(p)} className="sub-card" key={p.slug}><strong>{p.name}</strong><p>{p.intro}</p><span>{p.guide?"Läs guiden":"Visa sortiment"} →</span></Link>)}</div>}
